    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600 mt-1">Process payroll and manage employee payments</p>
        </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-slate-600">Certified Reports</p>
          ...
          <Card key={run.id} className="hover:shadow-lg transition-shadow">
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-xl">Payroll - {run.period}</CardTitle>
                  <p className="text-slate-600 mt-1">{run.employee_count} employees</p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="font-semibold text-lg">
                    {run.total_amount > 0 ? `$${run.total_amount.toLocaleString()}` : "Pending"}
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-semibold">
                    {run.run_date ? new Date(run.run_date).toLocaleDateString() : "Not run"}
                  </div>
                </div>
                <div>
                  <div className="text-sm text-slate-600">Pay Date</div>
                  <div className="font-semibold">{new Date(run.pay_date).toLocaleDateString()}</div>
                </div>
              </div>
            </CardContent>
          </Card>
